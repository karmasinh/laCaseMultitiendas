...
import type { ForumGeoSession, ForumPost, ForumProfile } from '../services/forum.api';
...
export const useForumStore = create<ForumStore>((set, get) => ({
  ...
  fetchProfile: async () => {
    try {
      const p = await api.getMyForumProfile();
      set({ profile: p });
    } catch {
      set({ profile: null });
    }
  ...
  redeemKarma: async (amount) => {
    ...
    set((s) => ({
      profile: s.profile ? { ...s.profile, karmaSpent: (s.profile.karmaSpent ?? 0) + amount } : s.profile,
    }));
    return res;
  },
  ...
  fetchGeoSession: async () => {
    try {
      ...
      if (geo.cityId && geo.city) get().fetchPosts({ reset: true, city: geo.city });
    } catch {
      set({ geoLoaded: true });
    }
  },
  ...
  updateGeo: async (payload) => {
    const res = await api.updateMyGeo(payload);
    set({ geo: res.profile });
    if (res.profile.cityId && res.profile.city) {
      get().fetchPosts({ reset: true, city: res.profile.city });
    }
  },
}));
